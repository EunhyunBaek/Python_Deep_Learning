...
def open_window():
    ...
    while (True):
        ...
        #cv2.imshow('gray_img', gray_img)  # 이미지 출력
        blur_img = gaussian_blur(gray_img, 3)  # Blur 효과
        #cv2.imshow('blur_img', blur_img)  # 이미지 출력
        canny_img = canny(blur_img, 70, 210)  # Canny edge 알고리즘
        cv2.imshow('canny_img', canny_img)  # 이미지 출력
        vertices = np.array(
        ...
        # cv2.imshow('line_arr', line_arr)  # 이미지 출력
        # 기울기 구하기
        ...
        print("Rl")
        print(R_lines)
        #if L_lines !=None and R_lines !=None:
        # 왼쪽, 오른쪽 각각 대표선 구하기
